...
export default function Tags() {
  ...
  async function handleCriar(e) {
    e.preventDefault();
    await api.post('/tags', { nome });
    setNome('');
    setCriando(false);
    setToast({ mensagem: 'Tag criada!', tipo: 'sucesso' });
    carregar();
  }

  async function handleEditar(e) {
    e.preventDefault();
    await api.put(`/tags/${editando}`, { nome: nomeEdicao });
    setEditando(null);
    setToast({ mensagem: 'Tag atualizada!', tipo: 'sucesso' });
    carregar();
  }

  async function handleExcluir() {
    await api.delete(`/tags/${confirmExcluir}`);
    setConfirmExcluir(null);
    setToast({ mensagem: 'Tag removida!', tipo: 'sucesso' });
    carregar();
  }
  ...
  return (
    <div className="adm">
        ...
        <div className="adm-header__left">
          <div className="adm-header__icon">
            <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
              <path d="M2 10V3C2 2.45 2.45 2 3 2H10L20 12L12 20L2 10Z" stroke="currentColor" strokeWidth="1.6" strokeLinejoin="round"/>
              <circle cx="7" cy="7" r="1.5" fill="currentColor"/>
            </svg>
          </div>
          <div>
            <h1 className="adm-header__title">Tags</h1>
      ...
      {/* Lista */}
      {filtradas.length === 0 && tags.length === 0 ? (
        <div className="adm-empty">
          <div className="adm-empty__icon">
            <svg width="40" height="40" viewBox="0 0 40 40" fill="none">
              <path d="M4 18V7C4 5.9 4.9 5 6 5H18L34 21L22 33L4 18Z" stroke="currentColor" strokeWidth="1.6" strokeLinejoin="round" opacity="0.3"/>
              <circle cx="12" cy="12" r="2" fill="currentColor" opacity="0.3"/>
              <path d="M24 28L30 34M30 28L24 34" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"/>
            </svg>
          </div>
          <h3>Nenhuma tag</h3>
          <p>Crie tags para classificar seus produtos.</p>
        ...
        <div className="adm-empty">
          <p>Nenhuma tag encontrada para "{busca}"</p>
        </div>
      ) : (
        <ul className="adm-list">
          {filtradas.map((tag, i) => (
            <li key={tag._id} className="adm-item">
              {editando === tag._id ? (
                ...
                <>
                  <div className="adm-item__left">
                    ...
                    <span className="adm-item__name">{tag.nome}</span>
                  </div>
                  <div className="adm-item__actions">
                    <button
                      className="adm-item__btn"
                      onClick={() => { setEditando(tag._id); setNomeEdicao(tag.nome); }}
                      aria-label="Editar"
                    >
                    ...
                    <button
                      ...
                      onClick={() => setConfirmExcluir(tag._id)}
                      aria-label="Excluir"
                    >
                      <svg width="15" height="15" viewBox="0 0 15 15" fill="none">
      ...
      <ConfirmDialog
        ...
        titulo="Excluir tag"
        mensagem="Tem certeza que deseja excluir esta tag? Ela será removida de todos os produtos associados."
